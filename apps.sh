...
# 🔥 NUEVO: Rutas para LDPlayer y MEmu Play
ldplayer_path1="${logged_in_home}/.local/share/Steam/steamapps/compatdata/NonSteamLaunchers/pfx/drive_c/Program Files/ldplayer/ldplayer.exe"
ldplayer_path2="${logged_in_home}/.local/share/Steam/steamapps/compatdata/LDPlayerLauncher/pfx/drive_c/Program Files/ldplayer/ldplayer.exe"
memu_path1="${logged_in_home}/.local/share/Steam/steamapps/compatdata/NonSteamLaunchers/pfx/drive_c/Program Files/Microvirt/MEmu/MEmu.exe"
memu_path2="${logged_in_home}/.local/share/Steam/steamapps/compatdata/MEmuPlayerLauncher/pfx/drive_c/Program Files/Microvirt/MEmu/MEmu.exe"
...
launcher_entries=(
  ...
  "$ldplayer_value|$ldplayer_text"  # 🔥 NUEVO: LDPlayer
  "$memu_value|$memu_text"          # 🔥 NUEVO: MEmu Play
...
function StartFreshFunction {
    ...
    rm -rf "/run/media/mmcblk0p1/LDPlayerLauncher/"    # 🔥 NUEVO
    rm -rf "/run/media/mmcblk0p1/MEmuPlayerLauncher/"  # 🔥 NUEVO
...
# 🔥 NUEVO: URLs para LDPlayer y MEmu Play
# LDPlayer
ldplayer_url="https://www.ldplayer.net/download/ldinst_9.0.69.1.exe"
ldplayer_file="${logged_in_home}/Downloads/NonSteamLaunchersInstallation/LDPlayerInstaller.exe"

# MEmu Play
memu_url="https://www.memuplay.com/download/memu-setup.exe"
memu_file="${logged_in_home}/Downloads/NonSteamLaunchersInstallation/MEmuInstaller.exe"
...
process_uninstall_options() {
    ...
    if [[ -n $uninstall_options ]]; then
        ...
        # 🔥 NUEVO: Uninstall LDPlayer
        if [[ $uninstall_options == *"Uninstall LDPlayer"* ]]; then
            uninstall_launcher "$uninstall_options" "LDPlayer" "$ldplayer_path1" "$ldplayer_path2" "${logged_in_home}/.local/share/Steam/steamapps/compatdata/NonSteamLaunchers/pfx/drive_c/Program Files/ldplayer" "${logged_in_home}/.local/share/Steam/steamapps/compatdata/LDPlayerLauncher" "ldplayer"
        fi

        # 🔥 NUEVO: Uninstall MEmu Play
        if [[ $uninstall_options == *"Uninstall MEmu Play"* ]]; then
            uninstall_launcher "$uninstall_options" "MEmu Play" "$memu_path1" "$memu_path2" "${logged_in_home}/.local/share/Steam/steamapps/compatdata/NonSteamLaunchers/pfx/drive_c/Program Files/Microvirt/MEmu" "${logged_in_home}/.local/share/Steam/steamapps/compatdata/MEmuPlayerLauncher" "memu"
        fi
...
if [ $# -gt 0 ]; then
    ...
    if [[ $options == "Uninstall" ]] || [[ $selected_launchers == "Uninstall" ]]; then
        ...
        uninstall_options=$(zenity --list --checklist \
            ...
            FALSE "STOVE Client" \
            FALSE "Moonlight Game Streaming" \
            FALSE "LDPlayer" \           # 🔥 NUEVO
            FALSE "MEmu Play" \          # 🔥 NUEVO
        )
...
# 🔥 NUEVO: LDPlayer specific installation steps
function install_ldplayer {
    terminate_processes "ldplayer.exe"
    # LDPlayer generalmente usa /S para instalación silenciosa
}

# 🔥 NUEVO: MEmu Play specific installation steps
function install_memu {
    terminate_processes "MEmu.exe"
    # MEmu generalmente funciona bien con /S
}
...
# 🔥 NUEVO: Install LDPlayer
install_launcher "LDPlayer" "LDPlayerLauncher" "$ldplayer_file" "$ldplayer_url" "$ldplayer_file /S" "97" "" "install_ldplayer" true

# 🔥 NUEVO: Install MEmu Play
install_launcher "MEmu Play" "MEmuPlayerLauncher" "$memu_file" "$memu_url" "$memu_file /S" "98" "" "install_memu" true
...
roots:
  ...
  - store: otherWindows
    path: ${logged_in_home}/.local/share/Steam/steamapps/compatdata/LDPlayerLauncher/pfx/drive_c/   # 🔥 NUEVO: LDPlayer
  - store: otherWindows
    path: ${logged_in_home}/.local/share/Steam/steamapps/compatdata/MEmuPlayerLauncher/pfx/drive_c/ # 🔥 NUEVO: MEmu Play
redirects: []
backup:
...
check_and_write "ldplayer" "$ldplayer_path1" "$ldplayer_path2" "NonSteamLaunchers" "LDPlayerLauncher" "" "ldplayer_launcher"  # 🔥 NUEVO
check_and_write "memu" "$memu_path1" "$memu_path2" "NonSteamLaunchers" "MEmuPlayerLauncher" "" "memu_launcher"  # 🔥 NUEVO
